...
std::pair<Point,NT> ComputeChebychevBall(const MT &A, const VT &b){
    ...
    try
    {
        // The following line is what was originally in the code before I (Long
        // Pham's) modified it. 
        // if (solve(lp) != OPTIMAL) throw false;

        // I need to reduce the accuracy threshold for compting Chebyshev balls.
        // Otherwise, we will get an accuracy error when we remove all implicit
        // equalities in the differential Bayesian resource analysis of
        // quickselect.
        ...
        int i = 0;
        while (status != OPTIMAL && i < 5) {
            status = solve(lp);
            i++;
        }

        if (status != OPTIMAL) {
            std::cout << "LP status for computing a Chebyshev ball is not OPTIMAL: " << get_statustext(lp, status) << std::endl;
            throw false;
        }
    }
    catch (bool e)
