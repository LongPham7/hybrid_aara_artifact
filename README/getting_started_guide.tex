...
In the initial working directory, we have a file \texttt{append.ml} that stores
the \texttt{append} function written in OCaml.
%
...
Hybrid RaML's executable is the file \texttt{/home/hybrid\_aara/raml/main}, and
...
%
Hybrid AARA (and its artifact Hybrid RaML) provides three types of hybrid
...
\end{verbatim}
%
In the command, \texttt{stat\_analyze} means we want to perform Hybrid AARA
(instead of the conventional AARA).
%
The input \texttt{ticks} means the resource metric of our interest is the tick
metric (instead of heap memory, for example).
%
The number 1 means we want to infer degree-1 polynomial cost bounds.
%
The input \texttt{append.ml} specifies the input file name, and the input
\texttt{append} means we want to know a cost bound of the function
\texttt{append} (instead of other functions in the same file).
%
Finally, the input \texttt{config\_opt.json} specifies the configuration file
for Hybrid \Opt{}.
...
\end{verbatim}
%
It means that the cost bound inferred by Hybrid \Opt{} is $0.85 \cdot M$, where
$M$ is the first input's length.
...
%
Its output on the stdout should contain
\begin{verbatim}
== append :
...
%
The output printed on the stdout should contain
\begin{verbatim}
== append :
...
(avg, std) = (         0,          0)  <--  []
  ...
  Simplified bound:
   ...
   where
     ...
     M is the number of ::-nodes of the 1st component of the argument
\end{verbatim}
where \texttt{0.164309 + 0.281939*L + 2.03295*M} contains the average
coefficients.
