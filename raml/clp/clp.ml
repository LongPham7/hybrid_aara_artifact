...
(* Write out a linear program in the MPS format *)
external write_Mps : t -> string -> unit = "clp_writeMps"
...
external change_column_lower : t -> float array -> unit
  = "clp_change_column_lower"

external column_upper : t -> float array = "clp_column_upper"

external change_column_upper : t -> float array -> unit
  = "clp_change_column_upper"

external objective_coefficients : t -> float array
  = "clp_objective_coefficients"
...
external primal_column_solution : t -> float array
  = "clp_primal_column_solution"
