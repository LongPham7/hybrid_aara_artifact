...
/*
 * val clp_writeMps : t -> unit;;
 */
CAMLprim value clp_writeMps(value model, value file_name)
{
    CAMLparam1 (model);
    Clp_Simplex* c_model = Model_val(model);
    char* c_file_name = String_val(file_name);
    Clp_writeMps(c_model, c_file_name, 0, 1, 0.0);
    CAMLreturn (Val_unit);
}
