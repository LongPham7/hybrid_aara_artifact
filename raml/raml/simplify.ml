...
let ident_prob      = Ident.create_persistent Rconfig.ocaml_prob_module
  ...
  and rec_type' t =
    ...
    | Types.Tconstr (Path.Pdot (Path.Pident mod_ident, "t", _), [ty], _)
                                  when Ident.same mod_ident ident_array
                                    -> Tarray (rec_type ty)
    | Types.Tconstr (Path.Pdot (Path.Pident mod_ident, "t", _), [], _)
                                  when Ident.same mod_ident ident_prob
                                    -> Tprob
...
let simplify_raml : ('a, 'b) expression -> string -> int -> ('a, 'b) expression_desc =
  fun e name line_num ->
    match name with
...
let simplify_prob : ('a, 'b) expression -> string -> ('a, 'b) expression_desc =
  fun e -> function
  | "create" -> Ebase_fun Prob_create
  | "flip" -> Ebase_fun Prob_flip
  | "consume" -> Ebase_fun (Prob_consume (0., 0., 0.))
  | "inv" -> Ebase_fun Prob_inv
  | "mult" -> Ebase_fun Prob_mult
  | s -> raise (Eunsupported_primitive (Rconfig.ocaml_prob_module ^ "." ^ s))
...
let rec simplify_exp : Typedtree.expression ->
  ...
  fun e ->
    ...
    | Typedtree.Texp_ident (Path.Pdot (Path.Pident mod_ident, op, _)
      ...
      try
        if Ident.same mod_ident ident_pervasive
          ...
          then simplify_raml template op e.Typedtree.exp_loc.Location.loc_start.Lexing.pos_lnum
        ...
        else if Ident.same mod_ident ident_prob
          then simplify_prob template op
        else
          raise Not_found
    ...
    | Typedtree.Texp_apply (f, ls)
      ...
      (* See simplify_prob *)
      | Ebase_fun (Prob_consume _), [{ exp_desc = Etuple [ {exp_desc = Ebase_const (Cfloat qh)} ; {exp_desc = Ebase_const (Cfloat qt)} ; {exp_desc = Ebase_const (Cfloat qc)} ] };a] ->
        Eapp (None,
            { f' with exp_desc = Ebase_fun (Prob_consume (qh, qt, qc)) ; exp_type = tapp f'.exp_type (Ttuple [Tbase Tfloat; Tbase Tfloat; Tbase Tfloat]) }, [a])
      ...
      (* Given a function application of the form Estat e1 a, if e1 is dummy,
      then we create Estat a. This is because the original function
      application in the source code must have been Raml.stat(a). Otherwise, if
      e1 is not dummy, the original function application in the source code must
      have been Raml.stat(e1) a, where e1 has an arrow type. In this case, we
      should create Eapp (Estat e1, a). *)
      | Estat e1, [a] when is_dummy e1 -> Estat a

      (* dereference can legitimately be at function fposition:
         let f = ref id in (!f) 1 *)
      ...
      | (Eref _ | Eref_assign _ | Ebase_fun (Nat_minusc _) |
         Enat_match _ | Etick _ | Ebase_fun (Prob_consume _)),
    ...
    | Typedtree.Texp_ifthenelse (if_, then_, else_) ->
      ...
      in
      let if' = simplify_exp if_ in
      begin
        match if'.exp_desc with
        | Eapp (_, {exp_desc = Ebase_fun Prob_flip}, [{exp_desc = Etuple [{exp_desc = Ebase_const (Cint a)}; {exp_desc = Ebase_const (Cint b)}]}]) ->
          Eflipc ((a,b), simplify_exp then_, else')
        | Eapp (_, {exp_desc = Ebase_fun Prob_flip; exp_kind; exp_info}, [{ exp_desc = Etuple [exp1; exp2] } as exp0]) ->
          Eflip ({ exp_desc = Eapp (None, { exp_desc = Ebase_fun Prob_create; exp_type = Tarrow ([Ttuple [Tbase Tint; Tbase Tint]], Tprob, ()); exp_kind = exp_kind; exp_info = exp_info }, [exp0]); exp_type = Tprob; exp_kind = if'.exp_kind; exp_info = if'.exp_info }, simplify_exp then_, else')
        | Eapp (_, {exp_desc = Ebase_fun Prob_flip}, [prob]) ->
          Eflip (prob, simplify_exp then_, else')
        | _ ->
          Econd (if', simplify_exp then_, else')
      end
...
let polymorphic_syntax =
  let rec f e = match e.exp_desc with
    ...
    | Econd (_, e1, e2) | Enat_match (_, e1, _, e2) | Eflip (_, e1, e2) | Eflipc (_, e1, e2) -> f e1 && f e2
    | Ematch (_, ls) -> List.for_all ls (fun (_, _, e') -> f e')

    | Ebase_const _
    | Evar _ | Elambda _ | Econst _ | Etuple _ | Eundefined | Etick _ | Estat _ -> true
...
let duplicate_instances e concrete_rtypes =
  ...
  let rec f e = match e.exp_desc with
    ...
    | Eflip (e1, e2, e3) ->
      let e2' = f e2 and e3' = f e3
      in { e with exp_desc = Eflip (e1, e2', e3'); exp_type = duplicated_type }
    | Eflipc (p, e2, e3) ->
      let e2' = f e2 and e3' = f e3
      in { e with exp_desc = Eflipc (p, e2', e3'); exp_type = duplicated_type }
    ...
    | Ebase_const _ | Evar _ | Elambda _ | Eundefined | Etick _ | Estat _ ->
...
let instantiate_polymorphism : typed_expression -> typed_expression =
  let rec inst_rec exp =
    ...
    | Eflip (e, e1, e2) -> Eflip (inst_rec e, inst_rec e1, inst_rec e2)
    | Eflipc (p, e1, e2) -> Eflipc (p, inst_rec e1, inst_rec e2)
...
let segment_exp_arrows exp =
  let rec mark_seg curr_symbol type_map e : segment_marked_expression =
    let desc, marked_type = match e.exp_desc with
      ...
      | Estat e1 ->  
        Estat (mark_seg curr_symbol type_map e1), mark_raml_type "stat" e.exp_type
      ...
      | Eflip (e0, e1, e2) ->
        let e0' = mark_seg "<Pmatch>" type_map e0 in
        let e1' = mark_seg curr_symbol type_map e1 in
        let e2' = mark_seg curr_symbol type_map e2 in
        let _ = unify_marked_type e1'.exp_type e2'.exp_type in
        Eflip (e0', e1', e2'), e1'.exp_type
      | Eflipc (p, e1, e2) ->
        let e1' = mark_seg curr_symbol type_map e1 in
        let e2' = mark_seg curr_symbol type_map e2 in
        let _ = unify_marked_type e1'.exp_type e2'.exp_type in
        Eflipc (p, e1', e2'), e1'.exp_type
  ...
  let rec seg_exp e : typed_expression =
    ...
    | Eflip (e1, e2, e3) -> Eflip (seg_exp e1, seg_exp e2, seg_exp e3)
    | Eflipc (p, e2, e3) -> Eflipc (p, seg_exp e2, seg_exp e3)
