...
let normalize_probs : typed_expression -> typed_expression =
	let transformer exp =
		match exp.exp_desc with
		| Eapp (_, { exp_desc = Ebase_fun Prob_create }, [ { exp_desc = Etuple [ { exp_desc = Ebase_const (Cint a) } ; { exp_desc = Ebase_const (Cint b) } ] } ]) ->
			{
				exp with
				exp_desc = Ebase_fun (Prob_createc (a, b))
			}
		| desc -> exp
	in
  ...
  fun prefix ->
	...
	| Eflip (e,e1,e2) ->
	  ...
	  share_vars2 en (en1,[]) (en2,[]) (
	    fun en' en1' en2' ->
	      let exp' =
		{ exp with exp_desc = Eflip (mk_var nv Tprob, en1', en2')
		  ; exp_info = Sln
		}
	      in
	      free_lets [nv] [en'] exp'
	...
	| Eflipc (p,e1,e2) ->
		let (en1, en2) = (norm e1, norm e2) in
		share_vars2 {exp with exp_desc = Eundefined} (en1, []) (en2, []) (
			fun en' en1' en2' ->
				let exp' = { exp with exp_desc = Eflipc (p, en1', en2'); exp_info = Sln } in
				exp'
		)
	...
	| Estat e ->
		(* let en = norm e in *)
		(* In Raml.stat(e), we do not convert e to the share-let normal form.
		Instead, we just recursively set exp_info inside the expression to Sln.
		This is done by the function Expressions.convert_typed_to_sln_expression. *)
		let en = convert_typed_to_sln_expression e in
		{ exp with exp_desc = Estat en
			; exp_info = Sln
		}
    in

    fun e ->
      let normalize = Fn.compose normalize_pattern_matches (Fn.compose normalize_nats normalize_probs) in
      norm (normalize e)
