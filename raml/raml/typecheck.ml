  ...
  let rec tcheck context exp =
	  ...
	  | Ref_swap -> (
 	  ...
 	  | Res_consume _ -> (
		...
		| Prob_flip ->
			...
			let f_str = string_of_builtin_fun f in
			failwith ("Built-in function " ^ f_str ^ " should not appear after simplification")

		| Prob_create -> check_fun t_exp [Ttuple [Tbase Tint; Tbase Tint]; Tprob]

		| Prob_createc _ -> t_exp == Tprob

		| Prob_consume _ -> check_fun t_exp [Tprob; Tprob]

		| Prob_inv -> check_fun t_exp [Tprob; Tprob]

		| Prob_mult -> check_fun t_exp [Ttuple [Tprob; Tprob]; Tprob]
	...
	; check_linear [e1,[] ; e2,xts]
      ...
      | Etick n -> t_exp == (Tbase Tunit)
			...
			| Estat e -> tcheck context e; e.exp_type == t_exp
...
let typecheck_stack ?linear:(linear=false) exp constr_map =
  ...
  let rec tcheck tstack_mode context exp =
	...
	; t_exp == (bounded_type e2')
	; List.iter2_exn e1'.exp_info e2'.exp_info (==)
	; { exp with exp_desc = Eflipc (p,e1',e2'); exp_info = e1'.exp_info; exp_type = e1'.exp_type }
