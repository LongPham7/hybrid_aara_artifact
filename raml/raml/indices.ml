...
let rec degree : index -> int =
 fun ind ->
  ...
  | Iunit -> 0
  | Inat n -> n
  | Ituple is -> List.fold is ~init:0 ~f:(fun acc i -> acc + degree i)
  | Iind ics ->
      let d = List.fold ics ~init:0 ~f:(fun acc (i, c) -> acc + degree i) in
      d + List.length ics

let rec zero_index : 'a rtype -> index =
 fun t ->
  match t with
  | Tbase _ | Tvar _ | Tarrow _ | Tref _ -> Iunit
  | Tprob -> Iprob (0, 0)
  | Tnat | Tarray _ -> Inat 0
...
let rec is_zero_index : index -> bool =
 fun ind ->
  match ind with
  | Iunit | Inat 0 | Iprob (0, 0) | Iind [] -> true
  | Ituple is -> List.for_all is is_zero_index
  | _ -> false

let map_all_zero : ('a, index, 'b) Map.t -> bool =
 fun ci -> Map.for_all ci is_zero_index

let list_all_zero : index list -> bool = fun is -> List.for_all is is_zero_index
...
type cindex = index list * index String.Map.t [@@deriving sexp, compare]

(* Converting a cindex into an index. *)
let index_of_cindex : cindex -> index =
 fun (is, cind) -> Ituple [ Ituple is; Ituple (Map.data cind) ]

let cdegree : cindex -> int = fun cind -> degree (index_of_cindex cind)

let zero_cindex : raml_type list * type_context -> cindex =
 fun (ts, context) -> (List.map ts zero_index, Map.map context zero_index)

let is_zero_cindex : cindex -> bool =
 fun (is, ci) -> List.for_all is is_zero_index && Map.for_all ci is_zero_index
...
let check_index :
    ('a rtype -> (index * Rtypes.constr_id) list -> bool) ->
    'a rtype ->
    index ->
    bool =
 fun check_ind_index t ind ->
  ...
  and valid_list ts is =
    ...
    | None -> false
    | Some tis -> List.for_all tis (fun (t, i) -> valid t i)
  and valid t ind =
    match ind with
    | Iunit -> (
        match t with Tbase _ | Tvar _ | Tref _ | Tarrow _ -> true | _ -> false )
    | Inat n -> ( match t with Tnat | Tarray _ -> true | _ -> false )
    | Ituple is -> ( match t with Ttuple ts -> valid_list ts is | _ -> false )
    | Iind ics -> (
        match t with
        | Tind cstr_list ->
            ...
            b1 && b2
        | _ -> false )
    | Iprob _ -> ( match t with Tprob -> true | _ -> false )
  in
...
let tracked_inductive_index =
  ...
  in
  let valid_inner t (i, cid) = constr_degree cid t > 0 in
  fun t ics ->
    match ics with
    | [] -> true
    | [ ic ] -> valid_leave t ic
...
let tracked_index : ?deg:int -> 'a rtype -> index -> bool =
 fun ?deg t i ->
  ...
  let b2 = match deg with None -> true | Some n -> n >= degree i in
  b1 && b2

let valid_cindex : 'b rtype list -> 'b rtype String.Map.t -> cindex -> bool =
 fun ts tcont (is, cind) ->
  let f ~key:var ~data:ind acc =
    acc
    &&
    match Map.find tcont var with None -> false | Some t -> valid_index t ind
  in
  Map.fold cind ~init:true ~f && valid_index (Ttuple ts) (Ituple is)

let tracked_cindex :
    ?deg:int -> 'b rtype list -> 'b rtype String.Map.t -> cindex -> bool =
 fun ?deg ts tcont (is, cind) ->
  ...
  && match deg with None -> true | Some n -> n >= cdegree (is, cind)
...
let distr_weight w k =
  let rec dweight prefix w k acc =
    ...
    | 0, 0 -> prefix :: acc
    | _, 0 -> acc
    | w, k -> process w prefix w k acc
  and process n prefix w k acc =
    if n = 0 then dweight (0 :: prefix) w (k - 1) acc
    else
...
let indices_deg :
    init:'a -> add_ind:(index -> 'a -> 'a) -> 'b rtype -> int -> 'a =
  ...
  let tuples ~acc ~add_ind fun_list f =
    let rec tup fun_list h =
      match fun_list with
      | [] -> h []
      | g :: gs -> g (fun i acc -> tup gs (fun is acc -> h (i :: is) acc) acc)
    in
    let add_ind is acc =
      match f is with Some i -> add_ind i acc | None -> acc
    in
  ...
  let rec inds_tuple g ts deg acc =
    ...
    List.fold dists ~init:acc ~f:(fun acc dist ->
        let ind_funs =
          List.map (List.zip_exn dist ts) ~f:(fun (k, t) f acc ->
              inds ~f t k acc)
        in
  ...
  and inds_ind g cts deg_out deg_in acc =
    ...
    let acc' =
      List.fold dists ~init:acc ~f:(fun acc dist ->
          ...
          in
          let check_val ics =
            if tracked_inductive_index (Tind cts) ics then Some (Iind ics)
            else None
          in
          tuples ~acc ~add_ind:g ind_funs check_val)
    in
    if deg_out = 0 then acc' else inds_ind g cts (deg_out - 1) (deg_in + 1) acc'
  and inds ~f t deg acc =
    match t with
    | Tbase _ | Tvar _ | Tarrow _ | Tref _ ->
        if deg = 0 then f Iunit acc else acc
    | Tprob ->
        let rec inner n1 acc =
          let n2 = deg - n1 in
          let acc' = f (Iprob (n1, n2)) acc in
          if n1 = 0 then acc' else inner (n1 - 1) acc'
        in
        inner deg acc
    | Tnat | Tarray _ -> f (Inat deg) acc
    | Ttuple ts -> inds_tuple f ts deg acc
    | Tind cts -> inds_ind f cts deg 0 acc
  in

  fun ~init ~add_ind t deg -> inds ~f:add_ind t deg init

(* indices of a degree smaller than or equal to a given deg *)
let indices_max_deg :
    empty:'a -> add_ind:(index -> 'a -> 'a) -> 'b rtype -> int -> 'a =
 fun ~empty ~add_ind t deg ->
  let indices_deg = indices_deg ~add_ind in

  let rec inds_max t deg acc =
    if deg = 0 then indices_deg ~init:acc t 0
    else
...
let rec pseudo_lin_num ind t =
  ...
  | _, _ -> raise (Invalid_argument "Index and type do not match.")

(* prioritized indices for objective function*)

let prioritized_indices t deg =
  let add_ind i acc = (i, degree i, pseudo_lin_num i t) :: acc in
  let inds = indices_max_deg ~empty:[] ~add_ind t deg in
  let cmp (i1, d1, l1) (i2, d2, l2) =
    assert (d1 >= l1);
    assert (d2 >= l2);
    let diff = d2 - l2 - (d1 - l1) in
    if diff = 0 then l1 - l2 else diff
  in
  let inds = List.sort ~compare:cmp inds in
  let break (i1, d1, l1) (i2, d2, l2) = not ((d1, l1) = (d2, l2)) in
  let iss = List.group ~break inds in
  let f is = List.map ~f:(fun (i, _, _) -> i) is in
  List.map ~f iss

(* The printing function for indices is copied here for debugging. It allows us
to print out indices in a module that "is used" by pprint.ml. Otherwise, we
would need to use the printing function in pprint.ml. But it cannot be used in a
...
let rename_cindex (free_variable_correspondence : (string * string) list)
    (cindex : cindex) : cindex =
  let list_indices, map_indices = cindex in
  let list_string_index = Map.to_alist map_indices in
  let rename_string_index (var_id, index) =
    match
      List.Assoc.find free_variable_correspondence ~equal:String.( = ) var_id
    with
    | None -> (var_id, index)
    | Some new_var_id -> (new_var_id, index)
  in
  let renamed_list_string_index =
    List.map list_string_index ~f:rename_string_index
  in
  (list_indices, String.Map.of_alist_exn renamed_list_string_index)
...
let print_cindex ?output:(formatter = std_formatter) cindex =
  let stack_indices, context_indices = cindex in
  let context_indices_list = Map.to_alist context_indices in
  pp_print_string formatter "stack: [";
  print_list_sep ~output:formatter stack_indices print_index "; ";
  pp_print_string formatter "]";
  pp_print_flush formatter ();
  pp_print_string formatter " context: [";
  print_list_sep ~output:formatter context_indices_list
    (fun (var_id, index) ->
      pp_print_string formatter var_id;
      pp_print_string formatter ": ";
      print_index ~output:formatter index)
    "; ";
  pp_print_string formatter "]";
  pp_print_flush formatter ()
