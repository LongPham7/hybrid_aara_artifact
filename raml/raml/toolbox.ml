...
let repeat : 'a -> int -> 'a list =
 fun a ->
  let rec rp acc n = if n <= 0 then acc else rp (a :: acc) (n - 1) in
  rp []

let iterate : int * int -> 'a -> (int -> 'a -> 'a) -> 'a =
 fun (low, high) start f ->
  let rec iter i acc = if i = high then acc else iter (i + 1) (f i acc) in
  iter low start

let sum = List.fold ~init:0 ~f:( + )

let p_map : ('a -> 'b) -> 'a * 'a -> 'b * 'b = fun f (a1, a2) -> (f a1, f a2)
...
let rec prefix l pref =
  ...
  | p :: ps -> (
      match l with
      | [] -> None
      | x :: xs -> if p = x then prefix xs ps else None )
...
let binomial n k =
  ...
  if k > n then 0 else if k > n - k then binom n (n - k) else binom n k

(*stirling numbers of the first kind*)

let rec pow n p =
  match p with 1 -> n | _ when p <= 0 -> 1 | _ -> n * pow n (p - 1)
...
let stirling n k =
  ...
  if k > n then 0
    ...
    let sign = if (n - k) mod 2 = 0 then 1 else -1 in
    sign * u_stirling n k
...
type 'a ref_chain = Rleaf of 'a | Rnode of 'a ref_chain ref

let rec rc_cycle address ref_chain =
  match ref_chain with
  | Rleaf _ -> false
  | Rnode ref_chain' ->
      if phys_equal address ref_chain' then true
      else rc_cycle address !ref_chain'

let rec rc_update f ref_chain =
  match !ref_chain with
  | Rleaf a ->
      let update = f a in
      if rc_cycle ref_chain update then () else ref_chain := update
  | Rnode ref_chain' -> rc_update f ref_chain'

let rec rc_get ref_chain =
  match !ref_chain with Rleaf a -> a | Rnode ref_chain' -> rc_get ref_chain'
  ...
  let multc_fast i j k =
    let rec sum l =
      ...
      | (r, s) :: facts ->
          sum facts
          + pow (-1) (i + j + r + s)
            * binomial i r * binomial j s
            * binomial (r * s) k
    in
    let f x (y, a) = (y, (x, y) :: a) in
    let g y a = snd (iterate (0, i + 1) (y, a) f) in
    sum (iterate (0, j + 1) [] g)
  in
...
let test_multc n =
  List.filter
    ~f:(fun ((i, j, k), a) -> a <> 0 && i + j > k)
    (List.map
       ~f:(fun (i, j, k) ->
         ((i, j, k), mult_coef i j k (* , multc_fast i j k *)))
       (let f j (i, k, a) = (i, k, (j, k, i) :: a) in
        let g i (k, a) =
          let _, _, a = iterate (0, n + 1) (i, k, a) f in
          (k, a)
        in
        let h k a = snd (iterate (0, n + 1) (k, a) g) in
        iterate (0, n + 1) [] h))
