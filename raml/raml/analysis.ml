...
module Make (Solver : SOLVER) (Amode : AMODE) = struct
  ...
  let amode_zero_out_tanno =
    ...
    | Mlower | Mconstant ->
        fun ?(spare_zero = false) tanno -> Anno.zero_out_tanno ~spare_zero tanno

  let amode_fresh_canno =
    match Amode.mode with
    | Mupper -> Anno.fresh_canno
    | Mlower | Mconstant -> Anno.zero_canno
  ...
  type fun_anno = {
    ...
    fan_map :
      fun_anno_type list ->
      fun_anno_type * (var type_anno -> metric -> var type_anno);
  }
  ...
  let fanno_to_raml_type ft =
    let rec trans ft =
      ...
      | Fref rc ->
          let ft = rc_get rc in
          Tref (trans ft)
      | Ftuple ts -> Ttuple (List.map ts trans)
      | Farrow fanno -> Tarrow (fanno.fan_targs, fanno.fan_tres, ())
      | Find clist ->
          let clist' =
            List.map clist (fun c -> { c with cstr_type = trans c.cstr_type })
          in
          Tind clist'
    in
    trans ft

  let unfold_ftype : fun_anno_type -> constr_id -> fun_anno_type =
   fun t cid ->
    match t with
    | Find clist -> (
        ...
        | Some d ->
            ...
            if deg > 0 then Ftuple (d.cstr_type :: Toolbox.repeat t deg)
            else if deg = 0 then d.cstr_type
            else raise (Analysis_error "Negative degree.") )
  ...
  let rec compare_fanno_types : fun_anno_type -> fun_anno_type -> bool =
   fun ft1 ft2 ->
    match (ft1, ft2) with
    | Fbase bt1, Fbase bt2 when bt1 = bt2 -> true
    | Fnat, Fnat -> true
    | Fprob, Fprob -> true
    | Fvar vid1, Fvar vid2 when vid1 = vid2 -> true
    | Farray rc1, Farray rc2 | Fref rc1, Fref rc2 ->
        compare_fanno_types (rc_get rc1) (rc_get rc2)
    | Ftuple t1s, Ftuple t2s ->
        List.for_all (List.zip_exn t1s t2s) (fun (t1, t2) ->
            compare_fanno_types t1 t2)
    | Farrow fanno1, Farrow fanno2 -> compare_fun_annos fanno1 fanno2
  ...
  type hybrid_aara_data_in_single_analysis =
    Location.t Runtime_cost_data.dataset_with_potential_categorized_by_indices
    option

  type analysis_arg = {
    anl_exp : analysis_expression;
    (* expression to be analyzed *)
    anl_metric : metric;
    (* a resource metric *)
    anl_deg : int;
    (*non-negative degree (redundant) *)
    anl_rtanno : var type_anno;
    (* matching type annotation for the return type *)
    anl_level : int;
    (* level of the expression, to collect function types for printing; 10000 -> disabled*)
    anl_sigma : fun_anno_context;
    (* a context with annotated function types *)
    anl_tstack : fun_anno_type list;
    (* a type stack with annotated function types *)
    anl_hybrid_aara_data : hybrid_aara_data_in_single_analysis;
  }
  ...
  let record_fun_type =
    ref
      (fun (n : int)
           (fid : string)
           (atanno : var type_anno)
           (rtanno : var type_anno)
           -> ())

  let record_consume_type =
    ref (fun (fid : string) (atanno : var type_anno) (rtanno : var type_anno) ->
        ())

  let recorded_fun_types =
    ref ([] : (string * var type_anno * var type_anno) list)

  let record_fun_max_level = 10000

  let init_type_collection types_mode =
    let () = recorded_fun_types := [] in
    ...
    let () =
      record_fun_type :=
        ...
        | Pconsume | Pnone -> fun _ _ _ _ -> ()
        | Plevel level ->
            fun n fid atanno rtanno ->
              if n <= level && n < record_fun_max_level then
                recorded_fun_types :=
                  (fid, atanno, rtanno) :: !recorded_fun_types
              else ()
  ...
  let unroll_fanno_type =
    let rec unroll ft =
      ...
      | Find clist ->
          Tind
            (List.map clist (fun c -> { c with cstr_type = unroll c.cstr_type }))
    in
    unroll
  ...
  let get_var e =
    ...
    | Evar x -> x
    | _ ->
        (* get_var fails when expression e is not in the let normal form. It can
        (but not always) happen when get_var handles e in Raml.stat(e) because,
        as of now, e inside Raml.stat(e) is not converted to the let normal
        form. A concrete example is concat with three branches: 

        let rec concat (xss : int list list) =
          match xss with 
          | [] -> [] 
          | [ hd ] -> hd 
          | hd :: tl -> append hd (Raml.stat (concat tl))
        
        In the third branch, after preprocessing (e.g. conversion of the input
        program to the share-let normal form), concat tl inside Raml.stat(...)
        takes the form concat (hd2 :: tl2), where tl = hd2 :: tl2, that is, tl
        is decomposed into hd2 :: tl2. Consequently, when get_var is applied to
        (hd2 :: tl2), it returns an error. To work around this problem, users
        need to rewrite RaML programs. *)
        (* For debugging *)
        (* let () =
          print_string
            "Expression e must be a variable, but it is not in the let normal \
             form: ";
          print_expression e;
          print_newline ()
        in *)
        raise (Analysis_error "Expression is not in let normal form.")
  ...
  and combine_fanno_types ft1 ft2 =
    match (ft1, ft2) with
    | Fbase bt1, Fbase bt2 when bt1 = bt2 -> Fbase bt1
    | Fnat, Fnat -> Fnat
    | Fprob, Fprob -> Fprob
    | Fvar vid1, Fvar vid2 when vid1 = vid2 -> Fvar vid1
  ...
  let combine_forward_results fr1 fr2 =
    match (fr1, fr2) with
    | Fclosed ft1, Fclosed ft2 -> Fclosed (combine_fanno_types ft1 ft2)
    ...
    | _, _ ->
        raise
          (Analysis_error "Forward result mismatch (open/closed) in branching.")
  ...
  let make_fanno_type analyze rtype =
    let rec make_fanno_type rtype =
      ...
      | Tind clist ->
          Find
            (List.map clist (fun c ->
                 { c with cstr_type = make_fanno_type c.cstr_type }))
    and make_fanno targs tres =
      let fan_map fts =
        let ft = make_fanno_type tres in
        (ft, analyze targs tres)
      in
      { fan_map; fan_targs = targs; fan_tres = tres }
    in
  ...
  let fold_fanno_type ft cid t_ind =
    ...
    | Tind clist -> (
        match List.find clist (fun d -> d.cstr_id = cid) with
        | None -> raise (Anno_exn "Constructor not part of type declaration.")
        | Some d -> (
            let deg = d.cstr_deg in
            if deg = 0 then
              let f c =
                if c.cstr_id = cid then
                  ...
                  { c with cstr_type = ft }
                else { c with cstr_type = unit_fanno_type c.cstr_type }
              in
              ...
              | Ftuple (ft1 :: ft2 :: fts) -> (
                  ...
                  let ft_ind = List.fold fts ~init:ft2 ~f:combine_fanno_types in
                  let f c =
                    if c.cstr_id = cid then
              ...
              | _ ->
                  raise
                    (Anno_exn
                       "Expection tuple type when folding a constructor of deg \
                        > 0.") ) )
        ...
        let fanno =
          let fan_map fts =
            ...
            | [ ft ] ->
                let ftres =
                  make_fanno_type
                    (fun _ -> raise (Analysis_error "Expecting a base type."))
                    tres
                in
                let analyze =
                  match targ with
                  | Ttuple [ Tnat; Tnat ] ->
                      fun rtanno metric ->
                        ...
                        let atanno =
                          Anno.fresh_tanno ~qzero deg (Ttuple [ targ ])
                        in
                ...
                in
                (ftres, analyze)
            | _ -> raise (Analysis_error "Expecting one function type.")
          in
          { fan_targs = [ targ ]; fan_tres = tres; fan_map }
        in
        Farrow fanno
  ...
  let bf_fanno_type bf bf_type =
    ...
    | Prob_create ->
        assert (
          bf_type = Tarrow ([ Ttuple [ Tbase Tint; Tbase Tint ] ], Tprob, ()) );
        let fanno =
          let fan_map fts =
            match fts with
            | [ Ftuple [ Fbase Tint; Fbase Tint ] ] ->
                let analyze rtanno metric =
                  assert (rtanno.tan_type = Tprob);
                  let () = Anno.zero_out_tanno ~spare_zero:true rtanno in
                  let cost = metric (Mbfun_eval Prob_create) in
                  Anno.constant_tanno cost
                    (Ttuple [ Ttuple [ Tbase Tint; Tbase Tint ] ])
                    rtanno
                in
                (Fprob, analyze)
            | _ ->
                raise
                  (Analysis_error
                     "Expecting function argument of type int * int.")
          in
          {
            fan_targs = [ Ttuple [ Tbase Tint; Tbase Tint ] ];
            fan_tres = Tprob;
            fan_map;
          }
        in
        Farrow fanno
    | Prob_createc _ -> raise (Analysis_error "Dead code.")
    | Nat_succ ->
        ...
        let fanno =
          let fan_map fts =
            ...
            | _ ->
                raise
                  (Analysis_error
                     "Expecting exectly one function type for built-in \
                      functions.")
          in
          { fan_targs = [ Tnat ]; fan_tres = Tnat; fan_map }
        in
        Farrow fanno
    | Nat_to_int ->
        ...
        let fanno =
          let fan_map fts =
            ...
            | _ ->
                raise
                  (Analysis_error "Expecting function argument of type nat.")
          in
          { fan_targs = [ Tnat ]; fan_tres = Tbase Tint; fan_map }
        in
        Farrow fanno
    | Arr_make -> (
        match bf_type with
        | Tarrow ([ Ttuple [ Tnat; t_elem ] ], Tarray t_elem', ())
          when t_elem = t_elem' ->
            let fanno =
              let fan_map fts =
                match fts with
                ...
                | _ ->
                    raise
                      (Analysis_error
                         "Expecting function argument of type nat and an \
                          element type.")
              in
              {
            ...
            let fanno =
              let fan_map fts =
                ...
                | _ ->
                    raise
                      (Analysis_error
                         "Expecting function argument of array type.")
              in
              { fan_targs = [ Tarray t_elem ]; fan_tres = Tnat; fan_map }
            in
            Farrow fanno
        | _ -> raise (Analysis_error "Wrong function type for Array.make.") )
    | Arr_set -> (
        match bf_type with
        | Tarrow ([ Ttuple [ Tarray t_elem; Tnat; t_elem' ] ], Tbase Tunit, ())
          when t_elem = t_elem' ->
            let fanno =
              let fan_map fts =
                match fts with
                | [ Ftuple [ Farray rc; Fnat; ft_elem ] ] ->
                    ...
                    let _ =
                      rc_update
                        (fun ft -> Rleaf (combine_fanno_types ft ft_elem))
                        rc
                    in
                    let analyze rtanno metric =
                      ...
                      let deg = rtanno.tan_deg in
                      let t_arg =
                        Ttuple [ Ttuple [ Tarray t_elem; Tnat; t_elem ] ]
                      in
    ...
    | Arr_get -> (
        match bf_type with
        | Tarrow ([ Ttuple [ Tarray t_elem; Tnat ] ], t_elem', ())
          when t_elem = t_elem' ->
            let fanno =
              let fan_map fts =
                match fts with
          ...
          let fan_map fts =
            ...
            | _ ->
                raise
                  (Analysis_error "Expecting function argument of type nat.")
          in
          { fan_targs = [ Tbase Tint ]; fan_tres = Tnat; fan_map }
        in
        Farrow fanno
    | Nat_of_intc n -> raise (Analysis_error "Dead code.")
    | Nat_add ->
          ...
          let fan_map fts =
            ...
            | _ ->
                raise
                  (Analysis_error "Expecting function argument of type nat.")
          in
          { fan_targs = [ Ttuple [ Tnat; Tnat ] ]; fan_tres = Tnat; fan_map }
        in
        Farrow fanno
    | Nat_mult ->
        assert (bf_type = Tarrow ([ Ttuple [ Tnat; Tnat ] ], Tnat, ()));
          ...
          let fan_map fts =
            ...
            | _ ->
                raise
                  (Analysis_error "Expecting function argument of type nat.")
          in
          { fan_targs = arg_ts; fan_tres = tres; fan_map }
        in
        Farrow fanno
    | Nat_div_mod ->
        let arg_ts = [ Ttuple [ Tnat; Tnat ] ] in
    ...
    | Res_consume line_num ->
          ...
          let fan_map fts =
            ...
            | [ ftarg ] ->
                let analyze rtanno metric =
                  ...
                  let gamma =
                    Anno.amode_consume_tanno rtanno (Ttuple [ targ ])
                  in
        ...
        in
        Farrow fanno
    | Prob_consume (qh, qt, qc) ->
        let tres = Tprob in
        let targ =
          match bf_type with
          | Tarrow ([ targ ], Tprob, ()) -> targ
          | _ -> raise (Analysis_error "Cannot match type for consume.")
        in
        assert (targ = Tprob);
        let fanno =
          let fan_map fts =
            ...
            | [ ftarg ] ->
                let analyze rtanno metric =
                  assert (rtanno.tan_type = tres);
                  let cost = metric (Mbfun_eval (Prob_consume (qh, qt, qc))) in
                  if not (phys_equal metric m_tick) then
                    let gamma =
                      Anno.amode_consume_prob_tanno (0., 0., 0.) rtanno
                        (Ttuple [ targ ])
                    in
                    Anno.add_cost_t gamma cost
                  else
                    let gamma =
                      Anno.amode_consume_prob_tanno (qh, qt, qc) rtanno
                        (Ttuple [ targ ])
                    in
                    Anno.add_cost_t gamma cost
                in
            ...
            | _ ->
                raise
                  (Analysis_error "Expecting function argument of type prob.")
          in
          { fan_targs = [ targ ]; fan_tres = tres; fan_map }
        in
        Farrow fanno
    | Prob_inv ->
        assert (bf_type = Tarrow ([ Tprob ], Tprob, ()));
        let fanno =
          let fan_map fts =
            match fts with
            | [ Fprob ] ->
                let analyze rtanno metric =
                  assert (rtanno.tan_type = Tprob);
                  let cost = metric (Mbfun_eval Prob_inv) in
                  let gamma = Anno.prob_inv rtanno in
                  Anno.add_cost_t gamma cost
                in
                (Fprob, analyze)
            | _ ->
                raise
                  (Analysis_error "Expecting function argument of type prob.")
          in
          { fan_targs = [ Tprob ]; fan_tres = Tprob; fan_map }
        in
        Farrow fanno
    | Prob_mult ->
        assert (bf_type = Tarrow ([ Ttuple [ Tprob; Tprob ] ], Tprob, ()));
        let fanno =
          let fan_map fts =
            match fts with
            | [ Ftuple [ Fprob; Fprob ] ] ->
                let analyze rtanno metric =
                  assert (rtanno.tan_type = Tprob);
                  let cost = metric (Mbfun_eval Prob_mult) in
                  let gamma = Anno.prob_mult rtanno in
                  Anno.add_cost_t gamma cost
                in
                (Fprob, analyze)
            | _ ->
                raise
                  (Analysis_error "Expecting function argument of type prob.")
          in
          { fan_targs = [ Ttuple [ Tprob; Tprob ] ]; fan_tres = Tprob; fan_map }
        in
        Farrow fanno
    | Nat_minusc _ | Ref_swap ->
        let f_str = string_of_builtin_fun bf in
        failwith ("Built-in function " ^ f_str ^ " not implemented yet.")
    | Prob_flip ->
        let f_str = string_of_builtin_fun bf in
        failwith
          ( "Built-in function " ^ f_str
          ^ " should not appear after simplification" )

  let unstack_type exp =
    let t_exp = exp.exp_type in
    match exp.exp_info with [] -> t_exp | ts -> Tarrow (ts, t_exp, ())

  let rec forward :
      fun_anno_context ->
      analysis_expression ->
      hybrid_aara_data_in_single_analysis ->
      forward_result =
   fun sigma exp hybrid_aara_data ->
    let tstack_exp = exp.exp_info in

    (* for debugging *)
    (* print_string "\nForward:\n";
    Pprint.print_expression exp;
    print_string "Variables in the context sigma: ";
    String.Map.iter_keys ~f:(fun x -> printf "%s " x) sigma;
    print_newline (); *)
    ...
    | Ebase_op op ->
        ...
        open_fanno_type tstack_exp ft
    | Evar x -> open_fanno_type tstack_exp (Map.find_exn sigma x)
    | Eapp (name, e, es) -> (
        let xs = List.map es get_var in
        match forward sigma e hybrid_aara_data with
        | Fopen op_ft ->
        ...
        | [] -> (
            let fan_map fts =
              ...
              | ft_x :: fts ->
                  ...
                  let ft_ret =
                    match forward sigma' e hybrid_aara_data with
                    | Fopen open_ftype -> open_ftype fts
                    | Fclosed ftype ->
                        assert (fts = []);
                        ftype
                  in
                    ...
                    let arg =
                      {
                        ...
                        anl_tstack = fts;
                        anl_sigma = sigma';
                        anl_hybrid_aara_data = hybrid_aara_data;
                      }
                    in
              ...
              | [] ->
                  raise
                    (Analysis_error
                       "Function annotations are expecting arguments.")
            in
            match exp.exp_type with
            | Tarrow (targs, tres, _) ->
                let fanno = { fan_map; fan_targs = targs; fan_tres = tres } in
        ...
        | _ ->
            let open_fanno fts =
              ...
              | [] ->
                  raise
                    (Analysis_error
                       "Open function annotations are expecting arguments.")
            in
            Fopen open_fanno )
    | Elet (x_opt, e1, e2) -> (
        assert (e1.exp_info = []);
        match x_opt with
        | None ->
            let _ = forward_closed sigma e1 hybrid_aara_data in
            (* side-effects of ref assign *)
            forward sigma e2 hybrid_aara_data
        | Some (x, _) ->
            let ft_x = forward_closed sigma e1 hybrid_aara_data in
            let sigma' = Map.set sigma x ft_x in
            forward sigma' e2 hybrid_aara_data )
    | Eletrec (xts, es, e) ->
        assert (List.fold es ~init:true ~f:(fun acc e -> e.exp_info = [] && acc));
        let fts = forward_rec_bind sigma xts es hybrid_aara_data in
        let xs = List.map xts fst in
        let sigma' = Toolbox.map_add_list sigma xs fts in
        forward sigma' e hybrid_aara_data
    | Econd (e, e1, e2) ->
        let fr1 = forward sigma e1 hybrid_aara_data in
        let fr2 = forward sigma e2 hybrid_aara_data in
        ...
        let forward_match (c, yts, e) =
          let fts_ys =
            ...
            | _, _ ->
                raise
                  (Analysis_error
                     "Typemismatch when undfolding of inductive type.")
          in
    ...
    | Enat_match (e, e1, (y, t_y), e2) ->
        ...
        let ft1 = forward sigma e1 hybrid_aara_data in
        let sigma' = Map.set sigma y Fnat in
        let ft2 = forward sigma' e2 hybrid_aara_data in
  ...
  and forward_closed sigma exp hybrid_aara_data =
    match forward sigma exp hybrid_aara_data with
    | Fclosed ft -> ft
  ...
  and forward_rec_bind :
    ...
    let equalize_fanno_types ft1 ft2 =
      if compare_fanno_types ft1 ft2 then ()
        ...
        raise
          (Analysis_error
             ( "Polymorphic recursion is currently not supported."
    ...
    let rec fix_annos rtype =
      ...
      | Tarrow (targs, tres, a) ->
          Tarrow (List.map targs fix_annos, fix_annos tres, ref Rempty)
      | Tind clist ->
          Tind
            (List.map clist (fun c ->
                 { c with cstr_type = fix_annos c.cstr_type }))
    in
    ...
    let update_fixed_rtype fixed_metric t_fixed t_body =
      let rec update_fixed t_fixed t_body =
        ...
        | Tarray t, Farray rc -> update_fixed t (rc_get rc)
        | Tref t, Fref rc -> update_fixed t (rc_get rc)
        | Ttuple t1s, Ftuple t2s -> List.iter2_exn t1s t2s update_fixed
        | Tarrow (targs, tret, rec_anno_ref), Farrow fanno_body -> (
            ...
            | _ ->
                raise
                  (Analysis_error
                     "Trying to update a let_rec_anno that is not empty.") )
    ...
    let rec contains_rused t_fixed =
      match t_fixed with
      | Tbase _ | Tnat | Tprob | Tvar _ -> false
      | Tarray t | Tref t -> contains_rused t
      | Ttuple ts -> List.exists ts contains_rused
      | Tind clist -> List.exists clist (fun c -> contains_rused c.cstr_type)
      | Tarrow (targs_fixed, tret_fixed, rec_anno_ref) ->
          ...
          let b3 = match !rec_anno_ref with Rused _ -> true | _ -> false in
          b1 && b2 && b3
    in

    let allow_rused = ref true in

    let make_fun_annos ?(foo = false) ?(poly_rec = false) fixed_metric max_deg
        t_fixed ft' =
      assert (max_deg >= 0);

      let rec rec_fannos t_fixed ft' =
        match (t_fixed, ft') with
        | Tbase bt1, Fbase bt2 when bt1 = bt2 -> Fbase bt1
        | Tnat, Fnat -> Fnat
        | Tprob, Fprob -> Fprob
        | Tvar vid1, Fvar vid2 when vid1 = vid2 -> Fvar vid1
        | Tarray t1, Farray rc ->
            let t2 = rc_get rc in
        ...
        | Tref t1, Fref rc ->
            ...
            let rc = ref (Rleaf (rec_fannos t1 t2)) in
            Fref rc
        | Ttuple t1s, Ftuple t2s -> Ftuple (List.map2_exn t1s t2s rec_fannos)
        | Tind clist1, Find clist2 ->
            let g c1 c2 =
              assert (c1.cstr_deg = c2.cstr_deg);
              assert (c1.cstr_id = c2.cstr_id);
            ...
            let fanno =
              let fan_map fts =
                ...
                let fa fixed_atanno fixed_rtanno rtanno met =
                  if rtanno.tan_deg < max_deg then fa' rtanno met
                  else (
                ...
                let ft, fa =
                  match !rec_anno_ref with
                  | Rempty ->
                      (*raise (Analysis_error "Trying to read empty let_rec_anno.")*)
                      ( rec_fannos tret_fixed ft',
                        if !allow_rused then fun rtanno met ->
                          if rtanno.tan_deg < max_deg then fa' rtanno met
                          else (
                            assert (phys_equal met fixed_metric);
                            assert (rtanno.tan_deg = max_deg);
                            let fixed_atanno =
                              Anno.fresh_tanno max_deg (Ttuple targs)
                            in
                            rec_anno_ref := Rused (fts, fixed_atanno, rtanno);
                            fixed_atanno )
                        else fun _ ->
                          raise
                            (Analysis_error "Trying to read empty let_rec_anno.")
                      )
                  | Rready fanno_body ->
                ...
                in
                let fa =
                  if (not poly_rec) || is_const_type (Ttuple targs) then fa
                  else fun rtanno met ->
                    let cf_deg = max_deg - 1 in
                    (* cf_deg is adjusted for the resource analysis of
                    median-of-medians-based linear-time selection algorithm. *)
                    (* let cf_deg = if foo then max_deg else max_deg - 1 in *)
                    ...
                    in *)
                    if
                      cf_deg = 0 || rtanno.tan_deg < max_deg
                      || phys_equal met m_costfree
                    then fa rtanno met
                    else (
                      assert (phys_equal met fixed_metric);
                      assert (rtanno.tan_deg = max_deg);
                      ...
                      let _ =
                        Anno.add_tannos ~tan:rtanno cf_rtanno rec_rtanno
                      in
                      Anno.add_tannos cf_atanno rec_atanno )
                in
                (ft, fa)
              in
              { fan_targs = targs; fan_tres = tres; fan_map }
            in
            Farrow fanno
    ...
    fun sigma xts es hybrid_aara_data ->
                  ...
                  (List.map clist (fun c ->
                       {
                         c with
                         cstr_type =
                           final_tanno c.cstr_type (get_fanno' c.cstr_id);
                       }))
            | Tarrow (targs, tret, ()) ->
                let fan_map fts =
                  let ft =
                    let get_fanno' max_deg metric =
                      match get_ftype max_deg metric with
                      | Farrow fanno' -> fst (apply_fanno fanno' fts)
                      | _ -> raise (Analysis_error "Type mismatch.")
                    in
                  ...
                  (ft, fa)
                in
                let fanno = { fan_targs = targs; fan_tres = tret; fan_map } in
                Farrow fanno
          in
      ...
      let rec mono_type_annos degree metric =
        ...
        let fts' = make_poly_types mono_type_annos in
        ...
        let make_fun_annos_save ?(poly_rec = false) fixed_t ft' =
          ...
          match xs with
          | f_name :: tl
            when Free_variable_correspondence.is_equal_function_names
                   "linear_select" f_name ->
              let ft =
                make_fun_annos ~foo:true ~poly_rec metric degree fixed_t ft'
              in
              assert (not (List.exists fixed_ts contains_rused));
              ft
          | _ ->
        ...
        let sigma' =
          let fts =
            List.map2_exn fixed_ts fts' (make_fun_annos_save ~poly_rec:true)
          in
          Toolbox.map_add_list sigma xs fts
        in

        let forward_rec e hybrid_aara_data =
          match forward sigma' e hybrid_aara_data with
          | Fclosed ft -> ft
        ...
        let _ =
          let f fixed_t e =
            update_fixed_rtype metric fixed_t (forward_rec e hybrid_aara_data)
          in
        ...
        let patch ft e =
          ...
          | Farrow fanno, Elambda ((x, t_x), e) ->
              let fan_map fts =
                ...
                | ft_x :: fts' ->
                    let sigma' = Map.set sigma x ft_x in
                    let _ = forward sigma' e hybrid_aara_data in
                    (* for the ref_assign side-effects *)
                    apply_fanno fanno fts
  ...
  and backward : analysis_arg -> var context_anno =
   fun arg ->
    let {
    ...
    let use_fun_type ft =
      ...
      let fun_name =
        match arg.anl_exp.exp_desc with Evar y -> y | _ -> "anonymous"
      in
    ...
    let get_context () =
      ...
      let f ~key:y ~data:ft_y =
        ...
        if Set.mem fvs (y, t_y) then Some t_y else None
      in
    ...
    if arg_deg = 0 then
      ...
      let the_canno =
        ...
        | Ebase_fun (Prob_createc (a, b)) ->
            assert (arg_tstack = []);
            assert (arg_rtanno.tan_type = Tprob);
            let gamma = Anno.prob_create (a, b) arg_rtanno in
            let cost_load = metric Mbfun_load in
            let cost_eval = metric (Mbfun_eval Prob_create) in
            let cost_ints = 2. *. metric Mbase_const in
            let cost_app = metric (Mapp 1) in
            let cost = cost_load +. cost_eval +. cost_ints +. cost_app in
            Anno.add_cost gamma cost
        | Ebase_fun basef -> (
            let cost_load = metric Mbfun_load in
            ...
            | _ ->
                raise
                  (Analysis_error
                     "Stack contains more than one argument in for built-in op.")
            )
        | Ebase_op op -> (
        ...
        | Elet (x_opt, e1, e2) -> (
            let combine_with_gamma1 gamma2 =
              let ind_zero =
                zero_cindex (gamma2.can_tstack, gamma2.can_context)
              in
            ...
            match x_opt with
            | Some (x, t_x) -> (
                let ft1 = forward_closed arg_sigma e1 hybrid_aara_data in
                let sigma' = Map.set arg_sigma x ft1 in
                let gamma2 =
                  backward { arg with anl_sigma = sigma'; anl_exp = e2 }
                in
                assert (
                  gamma2.can_tstack = List.map arg_tstack fanno_to_raml_type );
                ...
                | Some t_x ->
                        ...
                        if deg = 0 then
                          ...
                          let canno =
                            let level, anl_metric =
                              if deg = arg_deg then (arg_level + 1, arg_metric)
                              else (record_fun_max_level, m_costfree)
                            in
                            backward
                              {
                                arg with
                    ...
                    let context1 =
                      ...
                      | Left q ->
                          raise
                            (Analysis_error "Maximal degree seems to be zero.")
                    in
                    let gamma =
                      Anno.canno_let (tstack, context2) arg_deg context1
                        anno_map
                    in
                    Anno.add_cost gamma (metric Mlet) )
            | None ->
                let _ = forward_closed arg_sigma e1 hybrid_aara_data in
        ...
        | Eletrec (xts, es, e2) ->
            let fvs = free_vars arg.anl_exp in
            let context =
              (* using the variable types in e is a problem *)
              let f ~key:y ~data:ft_y =
                let t_y = unroll_fanno_type ft_y in
                if Set.mem fvs (y, t_y) then Some t_y else None
              in
            ...
            in
            let xs = List.map xts fst in
            let fts = forward_rec_bind arg_sigma xts es hybrid_aara_data in
        ...
        | Eapp (name, e, es) -> (
            ...
            in
            match forward arg_sigma e hybrid_aara_data with
            | Fopen open_ft -> (
                match arg_tstack with
                | [] ->
                ...
                | _ ->
                    ...
                    let gamma1 =
                      ...
                      assert (match t_e with Tarrow _ -> true | _ -> false);
                    ...
                    Anno.add_cost gamma cost )
            | Fclosed _ ->
                raise (Analysis_error "Expecting open forward result.") )
        | Econd (e, e1, e2) ->
            let x, t = (get_var e, e.exp_type) in
            let gamma1 =
              backward { arg with anl_exp = e1; anl_level = arg_level + 1 }
            in
            let gamma2 =
              backward { arg with anl_exp = e2; anl_level = arg_level + 1 }
            in
            let gamma1, gamma2 =
            ...
            in
            let gamma =
              Anno.combine_cannos_if ~add_var:(x, t) [ gamma1; gamma2 ]
            in
        ...
        | Eflip (e, e1, e2) ->
            ...
            in
            let gamma =
              Anno.combine_cannos_sym_prob ~on_var:(x, t) [ gamma1; gamma2 ]
            in
            Anno.add_cost gamma (metric Mflip)
        | Eflipc ((a, b), e1, e2) ->
            let gamma1 =
              backward { arg with anl_exp = e1; anl_level = arg_level + 1 }
            in
            ...
            in
            let gamma =
              Anno.combine_cannos_prob
                [
                  float_of_int a /. float_of_int (a + b);
                  float_of_int b /. float_of_int (a + b);
                ]
                [ gamma1; gamma2 ]
            in
            Anno.add_cost gamma (metric Mflip)
        | Enat_match (e, e1, (x_m, t), e2) ->
            ...
            let gamma1, gamma2 =
              match
                Anno.amode_patch_cannos [ gamma1; gamma2 ] [ []; [ x_m ] ]
              with
              | [ g1; g2 ] -> (g1, g2)
              | _ -> raise (Analysis_error "Dead code.")
            in
        ...
        | Ematch (e, matches) ->
            ...
            let gammas' = List.map2_exn conts gammas apply in
        ...
        | Eref_assign (e1, e2) ->
            ...
            let context = String.Map.of_alist_exn [ (x1, Tref t); (x2, t) ] in
            let gamma = amode_fresh_canno ~qzero arg_deg [] context in
        ...
        | Estat e -> (
            let tstack = arg.anl_exp.exp_info in
            let context = get_context () in
            (* Set a flag in Anno.S to true so that it now indicates we have
            encountered Raml.stat(...) *)
            let () = Anno.S.stat_aara_is_encountered () in
            (* print_endline "We are in the case of Estat";
            Pprint.print_expression ~print_types:true e; *)
            let is_cost_free_metric = metric (Mtick 1.) = 0. in
            (* For debugging *)
            (* if is_cost_free_metric then
              print_endline "We are in Estat with cost-free metric"
            else print_endline "We are in Estat with non-cost-free metric"; *)
            match hybrid_aara_data with
            (* None is matched when we perform the conventional AARA on an input
            program with Raml.stat(...). *)
            | None -> backward { arg with anl_exp = e }
            | Some runtime_data -> (
                match
                  Runtime_cost_data.find_dataset_associated_with_expression
                    runtime_data e
                with
                | None -> backward { arg with anl_exp = e }
                | Some runtime_data_of_e ->
                    (* For debugging *)
                    (* let () =
                      print_endline "The dataset associated with e:";
                      Hybrid_aara_pprint
                      .print_list_potential_categorized_by_indices
                        runtime_data_of_e
                    in *)
                    Anno.add_constraint_for_runtime_cost_data runtime_data_of_e
                      arg_deg tstack context arg_rtanno is_cost_free_metric ) )
      in
