...
let fprint_raml_type ?(indent = 2) ?(expand_ind = false)
    ?(nice_type_vars = true) f typ =
  (* nice names for type variables *)
  let fresh_var =
    let vars =
      ref [ "'a"; "'b"; "'c"; "'d"; "'e"; "'s"; "'t"; "'u"; "'v"; "'w" ]
    in
    let count = ref 0 in
    fun () ->
      match !vars with
  ...
  let rec fprint_constr : 'a. _ -> 'a rtype constructor -> _ =
   fun f c ->
  ...
  (* d: precedence
     ...
     _ -> _            = 0
  *)
  and fprint_type : 'a. _ -> _ * 'a rtype -> _ =
   fun f (d, typ) ->
    let open_paren f printing_prec =
      pp_open_box f indent;
      if d > printing_prec then pp_print_string f "("
    in
    ...
    | Tvar vt ->
        pp_print_string f
          (if nice_type_vars then mem_map vt.var_name else vt.var_name)
    | Tarray t ->
        fprintf f "%a%a@ array%a" open_paren 10 fprint_type (10, t) close_paren
          10
    | Tref t ->
        fprintf f "%a%a@ ref%a" open_paren 10 fprint_type (10, t) close_paren 10
    | Ttuple ts ->
        fprintf f "%a%a%a" open_paren 5 fprint_types_sep
          (6, ts, out_fixed " *@ ")
          close_paren 5
    | Tarrow ([ t1 ], t2, _) ->
        fprintf f "%a%a ->@ %a%a" open_paren 0 fprint_type (1, t1) fprint_type
          (0, t2) close_paren 0
    | Tarrow (ts, t, _) ->
        fprintf f "%a[%a] ->@ %a%a" open_paren 0 fprint_types_sep
          (0, ts, out_fixed ";@ ")
          fprint_type (0, t) close_paren 0
    | Tind [] -> fprintf f "%a{ }%a" open_paren 10 close_paren 10
    | Tind (c :: _ as constr_list) -> (
        let cstr_id, typs =
          match String.lsplit2 c.cstr_id '|' with
        ...
        | false, Some tycon -> (
            ...
            | [ typ ] ->
                fprintf f "%a%a@ %s%a" open_paren 10 fprint_type (10, typ) tycon
                  close_paren 10
            | typlist ->
                fprintf f "%a%a@ %s%a" open_paren 10 fprint_type
                  (10, Ttuple typlist) tycon close_paren 10 )
        | _, _ ->
            (* expand_ind = true || Map.find ... = None *)
            fprintf f "%a{@ %a@ }%a" open_paren 10 fprint_list_sep
              (constr_list, fprint_constr, out_fixed " |@ ")
              close_paren 10 )
  in

  fprint_type f (0, typ)

let print_raml_type ?output:(formatter = std_formatter) ?(indent = 2)
    ?(expand_ind = false) typ =
  fprintf formatter "%a@?"
    (fprint_raml_type ~indent ~expand_ind ~nice_type_vars:true)
    typ
...
let fprint_expression ?(indent = 2) ?(print_types = false) f exp =
  ...
  let fprint_binding f (x, t) =
    fprintf f "%s" x;
    if print_types then fprintf f " :@ %a" fprint_type t
  in
  ...
  and fprint_letrecs f xes =
    match xes with
    | [] -> ()
    | [ (xt, e) ] -> fprintf f "%a =@ %a" fprint_binding xt fprint_exp (0, e)
    | (xt, e) :: xes ->
        fprintf f "%a =@ %a@]@ @[and@ %a" fprint_binding xt fprint_exp (0, e)
          fprint_letrecs xes
  and fprint_matches f matches =
    ...
    | (constr, xts, e) :: matches ->
        printf "@]@ @[| %s %a@ ->@ %a%a"
          (if print_types then constr else List.hd_exn (String.split constr '|'))
          fprint_binding_list xts fprint_exp (0, e) fprint_matches matches
  ...
  and fprint_exp_desc f (d, desc) =
    ...
    | Eapp (name, e, es) -> (
        ...
        | Ebase_op op, [ { exp_desc = Etuple [ e1; e2 ] } ] when is_binop op ->
            ...
            let e1_prec, e2_prec =
              ...
              | None -> (prec + 1, prec + 1)
            in
            fprintf f "%a%a@ %a%s%a@ %a%a" open_paren (0, prec) fprint_exp
              (e1_prec, e1) set_color e.exp_kind (string_of_builtin_op op)
              set_color curr_color fprint_exp (e2_prec, e2) close_paren prec
        | _, _ ->
            let app_prec = 85 in
            fprintf f "%a%a@ %a%a" open_paren (0, app_prec) fprint_exp
              (app_prec + 1, e)
              fprint_exps_sep
              (app_prec + 1, es, Fn.flip pp_print_space ())
              close_paren app_prec )
    | Elambda (xt, e) ->
        let lambda_prec = 0 in
        fprintf f "%afun %a ->@ %a%a" open_paren (0, lambda_prec) fprint_binding
          xt fprint_exp (lambda_prec, e) close_paren lambda_prec
    | Elet (x_opt, e1, e2) ->
        let let_prec = 0 in
        fprintf f "%alet@ %a =@ %a@ in@]@ @[%a%a" open_paren (0, let_prec)
          fprint_binding_option x_opt fprint_exp
          (0, e1) (* inner exp has 0 precedence *)
          fprint_exp (let_prec, e2) close_paren let_prec
    | Eletrec (xs, es, e) ->
        let let_prec = 0 in
        fprintf f "%alet rec@ %a@ in@]@ @[%a%a" open_paren (0, let_prec)
          fprint_letrecs (List.zip_exn xs es) fprint_exp (let_prec, e)
          close_paren let_prec
    | Econd (e, e1, e2) ->
        let if_prec = 10 in
        fprintf f "%aif@ %a@ then@ %a@ @]%aelse@ %a%a" open_paren
          (indent, if_prec) fprint_exp (0, e) fprint_exp (0, e1) pp_open_box
          indent fprint_exp (if_prec, e2) close_paren if_prec
    | Eflip (e, e1, e2) ->
        let if_prec = 10 in
        fprintf f "%aflip@ %a@ then@ %a@ @]%aelse@ %a%a" open_paren
          (indent, if_prec) fprint_exp (0, e) fprint_exp (0, e1) pp_open_box
          indent fprint_exp (if_prec, e2) close_paren if_prec
    | Eflipc ((a, b), e1, e2) ->
        let if_prec = 10 in
        fprintf f "%aflipc@ (%d,%d)@ then@ %a@ @]%aelse@ %a%a" open_paren
          (indent, if_prec) a b fprint_exp (0, e1) pp_open_box indent fprint_exp
          (if_prec, e2) close_paren if_prec
    | Eshare (e1, x1, x2, e2) ->
        let share_prec = 0 in
        fprintf f "%ashare@ %a@ as@ (%a,@ %a)@ in@]@ @[%a%a" open_paren
          (0, share_prec) fprint_exp (0, e1) fprint_binding x1 fprint_binding x2
          fprint_exp (share_prec, e2) close_paren share_prec
    | Econst (c, e) ->
        let constr_prec = 70 in
        fprintf f "%a%s@ %a%a" open_paren (indent, constr_prec)
    ...
    | Ematch (e, matches) ->
        let match_prec = 0 in
        fprintf f "@[<2>%amatch@ %a@ with%a%a@]" open_paren (indent, match_prec)
          fprint_exp (0, e) fprint_matches matches close_paren match_prec
    | Enat_match (e, e1, xt, e2) ->
        let match_prec = 0 in
        fprintf f
          "@[<2>%amatch@ %a@ with@]@ @[| %s.%s ->@ %a@]@ @[| %s.%s %a ->@ \
           %a%a@]"
          open_paren (indent, match_prec) fprint_exp (0, e)
          Rconfig.ocaml_nat_module Rconfig.ocaml_nat_zero fprint_exp (0, e1)
          Rconfig.ocaml_nat_module Rconfig.ocaml_nat_succ fprint_binding xt
          fprint_exp (0, e2) close_paren match_prec
    | Eref e ->
        let ref_prec = 70 in
        fprintf f "%aref@ %a%a" open_paren (indent, ref_prec) fprint_exp
          (ref_prec + 1, e)
          close_paren ref_prec
    | Eref_deref e ->
        let deref_prec = 85 in
        fprintf f "%a!%a%a" open_paren (0, deref_prec) fprint_exp
          (deref_prec, e) close_paren deref_prec
    | Eref_assign (e1, e2) ->
        let assign_prec = 15 in
        fprintf f "%a%a@ :=@ %a%a" open_paren (indent, assign_prec) fprint_exp
          (assign_prec + 1, e1)
          fprint_exp (assign_prec, e2) close_paren assign_prec
    | Etuple es ->
        let always_paren = -1 in
        fprintf f "%a%a%a" open_paren (indent, always_paren) fprint_exps_sep
          (0, es, out_fixed ",@ ")
    ...
    | Etick q ->
        fprintf f "%s.%s(%f)" Rconfig.ocaml_raml_module Rconfig.ocaml_raml_tick
          q
    | Estat e ->
        fprintf f "%s.%s(%a)" Rconfig.ocaml_raml_module Rconfig.ocaml_raml_stat
          fprint_exp
          (70 + 1, e)
  in

  fprintf f "%a%a" fprint_exp (0, exp) set_color Enormal

let print_expression ?output:(formatter = std_formatter) ?(indent = 2)
    ?(print_types = false) exp =
...
let fprint_value ?(indent = 2) f (loc, heap) =
  let rec list_locs l =
    ...
    | Some (Vconst (c_id, l_cons))
      when String.is_prefix c_id (Rconfig.ocaml_list_cons ^ "|") -> (
        ...
        | Some (Vtuple [ l1; l2 ]) -> l1 :: list_locs l2
  ...
  let rec fprint_vals_sep f (indirect, ls, sep) =
    ...
    | l :: ls ->
        fprintf f "%a%t%a" fprint_val (false, indirect, l) sep fprint_vals_sep
          (indirect, ls, sep)
  ...
  and fprint_val f (parents, indirect, loc) =
    ...
    | Some value -> (
        ...
        | Vloc l ->
            if indirect then fprint_val f (false, false, l)
            else fprintf f "ref->{@ %a@ }" fprint_val (false, false, l)
        | Vtuple ls ->
            fprintf f "(@ %a@ )" fprint_vals_sep (false, ls, out_fixed ",@ ")
        | Vconst (c_id, l) ->
            if
              String.is_prefix c_id (Rconfig.ocaml_list_cons ^ "|")
              || String.is_prefix c_id (Rconfig.ocaml_list_nil ^ "|")
            then
              fprintf f "[@ %a@ ]" fprint_vals_sep
                (true, list_locs loc, out_fixed ";@ ")
            else (
              if parents then fprintf f "(@ ";
              fprintf f "%s@ %a"
                (List.hd_exn (String.split c_id '|'))
                fprint_val (true, true, l);
              if parents then fprintf f "@ )" )
        | Varray (arr, size) -> fprintf f "[|@ %a@ |]" fprint_array (size, arr)
        | Vprob (a, b) -> fprintf f "prob(%d,%d)" a b )
  in
...
let fprint_polynomial f p =
  ...
  | Some (min_key, _) ->
      ...
      let print_sign f q =
        ...
        else fprintf f "@ %s" (if q < 0. then "- " else "+ ")
      in
      Map.iteri p ~f:(fun ~key:basis ~data:q ->
          fprintf f "%a%g" print_sign q (Float.abs q);
          Map.iteri basis (fun ~key:var ~data:power ->
              fprintf f "*%s" var;
              if power <> 1 then fprintf f "^%d" power));
      pp_close_box f ();
      pp_print_flush f ()
...
let rec fprint_index f ind =
  ...
  | Iind clist ->
      let print_cind (ind, cid) =
        ...
        | _ -> fprintf f "%s(%a)" cid' fprint_index ind
      in
      fprintf f "[";
...
let print_cindex cindex =
  let stack_indices, context_indices = cindex in
  let context_indices_list = Map.to_alist context_indices in
  print_string "stack: [";
  print_list_sep stack_indices print_index "; ";
  print_string "]";
  pp_print_flush std_formatter ();
  print_string " context: [";
  print_list_sep context_indices_list
    (fun (var_id, index) ->
      print_string var_id;
      print_string ": ";
      print_index index)
...
let fprint_type_anno f tanno =
  ...
  let print_annos f tanno =
    let add_ind ind () =
      ...
      else fprintf f "@[%10g  <--  %a@]@ " q fprint_index ind
    in
...
let fprint_anno_funtype ?(indent = "") ?(simple_name = false) f
    (fid, atanno, rtanno) =
  let arrow_type =
  ...
  fprintf f "\n%sNon-zero annotations of the argument:\n%a" indent
    fprint_type_anno atanno;
  fprintf f "\n%sNon-zero annotations of result:\n%a" indent fprint_type_anno
    rtanno;
...
let print_anno_funtype ?(indent = "") ?(simple_name = false)
